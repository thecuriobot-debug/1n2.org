...
(function (a) {
    ...
    function j(d, b) {
        ...
        if (c == null) {
            ...
            a.each(a.jknav.items[b.name], function (e, g) {
            ...
            else if (d < 0 && f == Math.floor(a(a.jknav.items[b.name][c]).offset().top) && --c < 0) c = a.jknav.items[b.name].length - 1
        } else {
    ...
    function i(d, b) {
        if (d.target.tagName.toLowerCase() == "input") h("keyup: " + d.target.tagName + ", target is INPUT ignored.");
        else {
    ...
    a.jknav = {
        ...
        init: function (d) {
            var b = a.extend(a.extend({}, a.jknav.default_options), d);
...
function hl(item) {}
$(function () {
    $('h1').jknav(hl, 'h2h3').addClass('jknav');
    $.jknav.init({
        name: 'h2h3',
        easing: 'swing',
        speed: 100,
...
function add_new_content() {
  if($('h1:not(.jknav)').length > 0){
    $('h1:not(.jknav)').jknav(hl, 'h2h3').addClass('jknav');
  }
}
