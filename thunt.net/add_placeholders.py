#!/usr/bin/env python3
"""
Add movie posters using image URLs (no API needed)
Uses img.omdbapi.com with a working key or placeholders
"""
import re
...
def get_poster_url(title, year=""):
    """Generate poster URL using public CDN"""
    # Use placeholder.com with movie title
    # Or we can use a gradient with the title
    title_encoded = quote(title)
    return f"https://via.placeholder.com/300x450/800020/f5deb3?text={title_encoded}"

# Parse data
dvds, revs = [], []
for m in re.finditer(r"INSERT INTO dvd VALUES \('(.*?)', '(.*?)', '(.*?)', '.*?', '.*?', '.*?', (\d+)\);", sql):
    dvds.append({'name': esc(m[1]), 'dir': esc(m[2]), 'writer': esc(m[3]), 'poster': get_poster_url(esc(m[1]))})
...
for m in re.finditer(r"INSERT INTO thuntnet_reviews VALUES \((\d+), '(.*?)', '(.*?)', '(.*?)', '(.*?)', '(.*?)', '(.*?)', '(.*?)', (\d+)\);", sql, re.DOTALL):
    ...
    revs.append({
        ...
        'creator': m[7], 
        'date': m[9],
        'poster': get_poster_url(title, m[3])
    })
...
.dvd-poster { position: relative; background: linear-gradient(135deg, #800020 0%, #600010 100%); }
.review-poster { width: 100%; height: 220px; overflow: hidden; margin-bottom: 15px; border-radius: 8px; background: linear-gradient(135deg, #800020 0%, #600010 100%); }
...
<p style="font-size: 15px; color: #666; margin-bottom: 20px;">Complete collection from 2001</p>'''
...
<div class="dvd-poster"><img src="{dvd['poster']}" alt="{dvd['name']}" loading="lazy" onerror="this.style.display='none';this.parentElement.innerHTML='{dvd['name']}'"></div>
<div class="dvd-info">
...
<div class="review-poster"><img src="{rev['poster']}" alt="{rev['title']}" loading="lazy" onerror="this.style.display='none'"></div>
...
print(f"\n📊 Generated:")
print(f"   {len(dvds)} DVD posters")
print(f"   {len(revs)} review posters")
print(f"\n🎨 Using placeholder images with movie titles")
print(f"🎉 Pages updated with poster placeholders!")
