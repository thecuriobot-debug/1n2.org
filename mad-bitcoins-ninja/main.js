...
function normalizeWorld(stageData) {
  return {
    ...
    solids: (stageData.solids || []).map((solid) => ({ ...solid })),
...
function stageIdFromNumber(stageNumber) {
  return `stage${Math.min(6, Math.max(1, stageNumber))}`;
}

async function boot() {
  const params = new URLSearchParams(window.location.search);
  const requestedStage = Number(params.get("stage") || 1);
  const stageNumber = Number.isFinite(requestedStage) ? Math.max(1, Math.min(6, requestedStage)) : 1;
