...
function shouldFire(enemy, rangeToPlayer, verticalGap) {
  return enemy.attackTimer <= 0 && rangeToPlayer <= enemy.aggroRange && verticalGap < 46;
}
...
export class Enemy {
  constructor({ type, x, y }) {
    ...
    this.phaseTimer = randomRange(1.0, 1.8);
    this.dashCooldown = randomRange(0.8, 1.3);
  ...
  updateGroundChase(dx) {
    ...
    if (this.type === "hacker_ninja" && this.onGround && Math.abs(dx) < 68 && this.dashCooldown <= 0) {
      this.vx = this.direction * (this.speed + 52);
      this.dashCooldown = randomRange(0.9, 1.5);
      return;
    }
